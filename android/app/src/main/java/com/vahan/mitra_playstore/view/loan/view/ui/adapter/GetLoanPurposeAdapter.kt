...
import com.vahan.mitra_playstore.view.loan.view.ui.fragment.LoanApplicationFragment
...
class GetLoanPurposeAdapter(
    private val context: FragmentActivity,
    val activity: LoanApplicationFragment,
    private val getLoanPurposeList: MutableList<GetAmountAndPurposeDTO.Purpose>,
    private val getAmountList: MutableList<Int>,
    private val type: Int,
) : RecyclerView.Adapter<GetLoanPurposeAdapter.MyViewHolder>() {
        ...
        if (type == 0) {
            holder.binding.apply {
                ...
                fun setBackgroundOrange() {
                    if(getLoanPurposeList[position].key == key[0]){categoryImage.setImageResource(R.drawable.ic_icon_homeutilities)}
                    if(getLoanPurposeList[position].key == key[1]){categoryImage.setImageResource(R.drawable.ic_icon_houserent)}
                    if(getLoanPurposeList[position].key == key[2]){categoryImage.setImageResource(R.drawable.ic_icon_family)}
                    if(getLoanPurposeList[position].key == key[3]){categoryImage.setImageResource(R.drawable.ic_icon_vehicle)}
                    if(getLoanPurposeList[position].key == key[4]){categoryImage.setImageResource(R.drawable.ic_icon_shopping)}
                    if(getLoanPurposeList[position].key == key[5]){categoryImage.setImageResource(R.drawable.ic_icon_festival)}
                    if(getLoanPurposeList[position].key == key[6]){categoryImage.setImageResource(R.drawable.ic_icon_medical)}
                    if(getLoanPurposeList[position].key == key[7]){categoryImage.setImageResource(R.drawable.ic_icon_other)}
